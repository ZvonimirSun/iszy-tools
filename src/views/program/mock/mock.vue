...
async function deletePrj (prj: MockPrj) {
  try {
    await axios.delete(`${axios.$apiBase}/mock/api/prj/${prj.id}`)
...
function getMockPrjs () {
  axios.get(`${axios.$apiBase}/mock/api/prj/list`).then(axios.getData).then((data: {
    success: boolean
    data?: MockPrj[]
  }) => {
    if (data.success) {
