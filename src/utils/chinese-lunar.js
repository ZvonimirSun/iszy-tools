 ...
 * 分析日期表达式，并提取其中的单位和数值
 */
const _expression = function (expr) {
  const list = expr.match(/[+-]?\d+((ms)|[yMdhmsw])/g)
  const result = []
...
// 计算公历两个日期之差
const _solarDiff = function (left, right, interval) {
...
_chineseLunar.dateDiff = function (lunar1, lunar2, expr) {
  ...
  const solar1 = _chineseLunar.lunarToSolar(lunar1)
  const solar2 = _chineseLunar.lunarToSolar(lunar2)
  // 再把农历转到公历
...
_chineseLunar.monthAdd = function (lunar, inc) {
  ...
  // 获取最后的结果年的闰月是哪一个月
  const leapMonth = _chineseLunar.leapMonthOfYear(year)
  let leap = false
  // 如果闰月大于农历月，则月份减1
...
_chineseLunar.monthDiff = function (lunar1, lunar2) {
  ...
  if (lunar1.year === lunar2.year) {
    ...
    // 计算两年之间中间的年月份
    let year = lunar1.year + 1
    for (let i = year; i < lunar2.year; i++) {
      count += _chineseLunar.monthsOfYear(year++)
    }
    // 计算最后一年
    count += lunar2.month
...
_chineseLunar.daysOfMonth = function (year, month, leap) {
  ...
  const date1 = _chineseLunar.lunarToSolar(year, month, 1, leap)
  const leapMonth = _chineseLunar.leapMonthOfYear(year)
...
_chineseLunar.lunarToSolar = function (year, month, day, leap) {
  ...
  // 取得润月是哪一个月
  const leapMonth = MAPPING[offset][0]
  ...
  // 取出对应到某个月的片断
  const segment = MAPPING[offset][month] // 农历第一天对应公历的具体天
  let mapMonth = Number(segment.substring(0, 2))
  const mapDate = Number(segment.substring(2, 4))
  ...
  const solar = new Date(year, mapMonth - 1, mapDate)
  const time = solar.getTime() + (day - 1) * 24 * 60 * 60 * 1000
...
_chineseLunar.solarToLunar = function (solar, format) {
  const offset = solar.getFullYear() - MIN_YEAR
  // 超出范围
  if (offset <= 0 || offset >= MAPPING.length) {
    throw new Error('Specified date range is invalid.')
  }
  ...
  // 查找范围内的农历数据
  let data = _findLunar(solar, offset, 0, 13, false)
  ...
  const day = _solarDiff(solar, firstDay, 'd') + 1

  // 返回的农历结果
  const result = new Lunar({
...
_chineseLunar.yearName = function (year) {
  const cnStr = '〇,一,二,三,四,五,六,七,八,九'.split(',')
...
_chineseLunar.monthName = function (month, traditional, leap) {
  const monthName = '正,二,三,四,五,六,七,八,九,十,十一,十二'.split(',')
  if (traditional) {
    monthName[11] = '腊'
  }
